...
public class ExampleUsage : MonoBehaviour {
    ...
    bool IsGoodWindow(WindowCapture window)
    {
        ...
        if (windowLowerTitle.Contains("blt0"))
        {
            return true;
        }
        return false;
    ...
    void OnAddWindow(WindowCapture window)
    {
        if (!windowsRendering.ContainsKey(window.hwnd) && IsGoodWindow(window))
        {
            ...
            windowObject.transform.localEulerAngles = new Vector3(104.19f, 8.9f, 0.33f);
			windowObject.transform.position = new Vector3(-0.319f,1.003f,0.516f);
